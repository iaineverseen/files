...
local Icons = {
    Menu = 'rbxassetid://105384072434825',
    Minimize = 'rbxassetid://75810158557243',
...
local ExistingHub = CoreGui:FindFirstChild('Nexus Container')
if ExistingHub then ExistingHub:Destroy() end

local NexusContainer = CreateElement('Folder', {
...
local MenuGui = CreateElement('ScreenGui', {
    Name = 'Nexus Button Menu',
    Parent = NexusContainer,
...
local MenuOutline = CreateElement('Frame', {
    ...
    Position = UDim2New(0.02, 0, 0.02, 0),
...
local NotificationContainer = CreateElement('Frame', {
    ...
    Position = UDim2New(1, -20, 0, 2),
    Size = UDim2New(0, 280, 1, -20),
    AnchorPoint = Vec2(1, 0)
})

local NotificationList = CreateElement('UIListLayout', {
...
function Library:Notify(options)
    ...
    local message = options.Message or options.message or ''
    local duration = options.Duration or options.duration or 3
    local notifType = options.Type or options.type or 'Info'
    ...
    local notifColor = typeColors[notifType] or typeColors.Info
    local notifIcon = typeIcons[notifType] or typeIcons.Info
    ...
    local NotifBorder = CreateElement('UIStroke', {
        Parent = NotifFrame,
        Color = notifColor,
        Thickness = 2,
        Transparency = 0.3
    ...
    task.spawn(function()
        ...
        local titleHeight = titleBounds.Y + 4
        local messageHeight = messageBounds.Y + 4
        local padding = 16
        local calculatedHeight = titleHeight + messageHeight + padding
        
        if calculatedHeight > 65 then
            local newTitleHeight = math.max(18, titleHeight)
            local newMessageHeight = math.max(24, messageHeight)
            local totalHeight = newTitleHeight + newMessageHeight + padding
            
            TitleLabel.Size = UDim2New(1, 0, 0, newTitleHeight)
            MessageLabel.Position = UDim2New(0, 0, 0, newTitleHeight + 4)
            MessageLabel.Size = UDim2New(1, 0, 0, newMessageHeight)
            NotifFrame.Size = UDim2New(1, 0, 0, totalHeight)
            
            if NotificationList then
                NotificationList:ApplyLayout()
            end
        end
    end)
    
    CreateTween(NotifFrame, 0.5, {
        Position = UDim2New(1, 0, 0, 0),
        BackgroundTransparency = 0
    }):Play()
    
    task.delay(duration, function()
        CreateTween(NotifFrame, 0.4, {BackgroundTransparency = 1}):Play()
        CreateTween(NotifBorder, 0.4, {Transparency = 1}):Play()
        CreateTween(Icon, 0.4, {ImageTransparency = 1}):Play()
        CreateTween(TitleLabel, 0.4, {TextTransparency = 1}):Play()
        CreateTween(MessageLabel, 0.4, {TextTransparency = 1}):Play()
        
        CreateTween(NotifFrame, 0.4, {
            Position = UDim2New(1, 50, 0, 0)
        }):Play()
...
function Library:CreateWindow(title)
    ...
    Connections[#Connections + 1] = TabListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
        TabScroll.CanvasSize = UDim2New(0, 0, 0, TabListLayout.AbsoluteContentSize.Y + 10)
    end)
    ...
    function Window:CreateTab(name)
        ...
        function Tab:AddDropdown(text, defaultValue, options, callback)
            ...
            local NoOptions = CreateElement('TextLabel', {
                Name = 'NoOptionsLabel',
                Parent = DropScroll,
                BackgroundTransparency = 1,
            ...
            NoOptions.Visible = CountOptions() == 0
            UpdateText()
            UpdateVisuals()
